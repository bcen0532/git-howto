...
gitではsvnと違い、ローカルで作業をコミットした後、それをリモート（サーバー側）に送信する
という形になります。

(厳密に言うとローカルとサーバーの区別はありません。単に相手のリポジトリにPUSHする権限があるかどうかだけです）。
...
上の様にすると、testディレクトリ内に.git/ディレクトリが作られます。ここがリポジトリになります。
...
と言う風になっています。
コマンドだけ使う場合にはあまり意識する必要はありませんが、gitで管理するファイル・履歴・ブランチなどは全てここに格納されています。
逆に言うとこの.git以下をコピーするとそれだけでリポジトリ全体をコピーできます（後述）。
...
となり、内容が追加されているのがわかります。
これはzlibでhoge.txtを圧縮したものです。

hogehogeと言う名前はファイル名 + コンテンツ をハッシュ関数(SHA1で)求めた文字列で、ファイル名とファイル内容から
（現実的には）一意に求まります。
...
コミットされたファイルを削除することが出来ます。

> $ git rm hoge.txt

git rmにより以下の処理が行われます。

index内からhoge.txtへの参照が消され、ディレクトリ内のhoge.txtファイルが削除されます。


## ファイルの変更

ファイルの変更を登録するのもgit addを使用します。
